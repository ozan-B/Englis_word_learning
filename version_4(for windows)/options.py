...
from options_interface import Ui_MainWindow
import pandas as pd
import random
import os
import pyttsx3
...
class Eng_turk_Page(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.radioButton_siraylaSor.clicked.connect(self.baslangic_index_sifirla)  # Radyo düğmesini indeksi sıfırlamak için bağla
        self.radioButton_siraylaSor.clicked.connect(self.ekran_kelime_yaz_sirayla)

        self.toolButton_back.clicked.connect(self.hide_page)
    ...
    def baslangic_index_sifirla(self):
        # Geçerli indeksi sıfırla
        self.current_index = 0

    def ekran_kelime_yaz_sirayla(self):
        if self.veri is not None:
            

            # Eğer listenin sonuna gelindi ise current_index'i sıfırla
            if self.current_index >= len(self.veri):
                self.current_index = 0

            # Geçerli indeksi kullanarak kelimeyi al
            self.secilen_index = self.current_index
...
# Sorulan kelimeyi textBrowser'a göster
    ...
    def Cevapla(self):
        ...
        if self.radioButton_siraylaSor.isChecked():
             # Doğru cevap kontrolü
            dogru_cevap = self.veri.loc[self.secilen_index, self.turkce_sutun] #Bu satır, doğru cevabın, seçilen kelimenin Türkçe karşılığını (self.turkce_sutun) içeren veri çerçevesinden (self.veri) alır ve dogru_cevap adlı bir değişkene atar. İşte daha ayrıntılı açıklama:
             #.loc[self.secilen_index, self.turkce_sutun]: Bu ifade, self.veri veri çerçevesinin belirli bir konumundan veri seçer. self.secilen_index indeksi ve self.turkce_sutun sütun adı kullanılarak, seçilen kelimenin Türkçe karşılığını (dogru_cevap) alır.
    ...
    def open_file_dialog(self):
        ...
        if file_name:
            ...
            # Uzantıyı küçük harfe dönüştür ve kontrol et
            uzanti = uzanti.lower()
                               
            
               
            if uzanti == '.xlsx' or uzanti == '.ods':
    ...
    def baslik_yaz(self):
        # Yanlış cevap durumunda metni oluştur
        metin = f"<div style='text-align: center;'><font color='red' style='font-size: 20px;'></font><span style='color: yellow; font-size: 20px; font-weight: bold;'>İnglizce-Türkçe</span></div><font>"
    ...
    def help(self):
        ...
        message_box.setText("Excel dosyanızda iki sütun olacak .Birinci sütunun adı 'English' , ikinci Sütunun adı 'Turkish' olmalıdır. Program size ingilizce kelimeyi verecek siz de türkçe karşılığını cevap olarak yazıcaksınız.")
        message_box.exec_()
...
class Turk_eng_Page(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        super().__init__()       #super()fonksiyonu, üst sınıfın (QtWidgets.QMainWindow) metotlarına erişimi sağlar. __init__ metodu, üst sınıfın yapıcı metodunu çağırarak, üst sınıfın başlatılmasını gerçekleştirir.Bu, Main sınıfının QtWidgets.QMainWindow sınıfının özelliklerini ve davranışlarını devralmasını sağlar.
        self.setupUi(self)      #Ui_MainWindow sınıfının setupUi metodunu çağırarak kullanıcı arayüzünün kurulumunu gerçekleştirir.
    ...
    def ekran_kelime_yaz_rastgele(self):
        ...
        if self.veri is not None:
            ...
            #Seçilen indexteki türkçe kelimeyi al
            self.sorulan_kelime = self.veri.loc[self.secilen_index, self.turkce_sutun]
    ...
    def ekran_kelime_yaz_sirayla(self):
        if self.veri is not None:
            ...
            self.sorulan_kelime = self.veri.loc[self.current_index, self.turkce_sutun]
...
# Sorulan kelimeyi textBrowser'a göster
    ...
    def Cevapla(self):
        ...
        if self.radioButton_siraylaSor.isChecked():
             # Doğru cevap kontrolü
            dogru_cevap = self.veri.loc[self.current_index, self.ingilizce_sutun] #Bu satır, doğru cevabın, seçilen kelimenin Türkçe karşılığını (self.turkce_sutun) içeren veri çerçevesinden (self.veri) alır ve dogru_cevap adlı bir değişkene atar. İşte daha ayrıntılı açıklama:
             #.loc[self.current_index, self.turkce_sutun]: Bu ifade, self.veri veri çerçevesinin belirli bir konumundan veri seçer. self.secilen_index indeksi ve self.turkce_sutun sütun adı kullanılarak, seçilen kelimenin Türkçe karşılığını (dogru_cevap) alır.
            ...
            # Doğru cevap kontrolü
            dogru_cevap = self.veri.loc[self.secilen_index, self.ingilizce_sutun] #Bu satır, doğru cevabın, seçilen kelimenin Türkçe karşılığını (self.turkce_sutun) içeren veri çerçevesinden (self.veri) alır ve dogru_cevap adlı bir değişkene atar. İşte daha ayrıntılı açıklama:
             #.loc[self.secilen_index, self.turkce_sutun]: Bu ifade, self.veri veri çerçevesinin belirli bir konumundan veri seçer. self.secilen_index indeksi ve self.turkce_sutun sütun adı kullanılarak, seçilen kelimenin Türkçe karşılığını (dogru_cevap) alır.
    ...
    def oku(self):
        ...
        """VOICE"""
        voices = engine.getProperty('voices')       
        engine.setProperty('voice',voices[4].id) #4.ses motoru kulağıma hoş geldi daha sonra değiştirebilirm.
    ...
    def baslik_yaz(self):
        # Yanlış cevap durumunda metni oluştur
        metin = f"<div style='text-align: center;'><font color='red' style='font-size: 20px;'></font><span style='color: yellow; font-size: 20px; font-weight: bold;'>Türkçe-İngilizce</span></div><font>"
    ...
    def help(self):
        ...
        message_box.setIcon(QMessageBox.Information)
        message_box.setWindowTitle("Bilgi")
        message_box.setText("Excel dosyanızda iki sütun olacak .Birinci sütunun adı 'English' , ikinci Sütunun adı 'Turkish' olmalıdır. Program size Türkçe kelimeyi verecek siz de İngilizce karşılığını cevap olarak yazıcaksınız.")
        message_box.exec_()           
...
class Ucuncu_hal_Page(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        """self.toolButton_back.clicked.connect(self.open_page_anasayfa)
                                self.pencere_anasayfa = Anasayfa()"""

        #self.anasayfa_acc = Anasayfa()

    """def open_page_anasayfa(self):
                    self.pencere_anasayfa.show()"""
    def hide_page(self):
        self.hide()
...
# Sorulan kelimeyi textBrowser'a göster
    ...
    def Cevapla(self):
        ...
        if self.radioButton_siraylaSor.isChecked():
             # Doğru cevap kontrolü
            dogru_cevap = self.veri.loc[self.secilen_index, self.ucuncu_hal_sutun] #Bu satır, doğru cevabın, seçilen kelimenin Türkçe karşılığını (self.turkce_sutun) içeren veri çerçevesinden (self.veri) alır ve dogru_cevap adlı bir değişkene atar. İşte daha ayrıntılı açıklama:
             #.loc[self.secilen_index, self.turkce_sutun]: Bu ifade, self.veri veri çerçevesinin belirli bir konumundan veri seçer. self.secilen_index indeksi ve self.turkce_sutun sütun adı kullanılarak, seçilen kelimenin Türkçe karşılığını (dogru_cevap) alır.
...
class Ikinci_hal_Page(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        super().__init__()       #super()fonksiyonu, üst sınıfın (QtWidgets.QMainWindow) metotlarına erişimi sağlar. __init__ metodu, üst sınıfın yapıcı metodunu çağırarak, üst sınıfın başlatılmasını gerçekleştirir.Bu, Main sınıfının QtWidgets.QMainWindow sınıfının özelliklerini ve davranışlarını devralmasını sağlar.
        self.setupUi(self)      #Ui_MainWindow sınıfının setupUi metodunu çağırarak kullanıcı arayüzünün kurulumunu gerçekleştirir.
...
# Sorulan kelimeyi textBrowser'a göster
    ...
    def Cevapla(self):
        ...
        if self.radioButton_siraylaSor.isChecked():
             # Doğru cevap kontrolü
            dogru_cevap = self.veri.loc[self.secilen_index, self.ikinci_hal_sutun] #Bu satır, doğru cevabın, seçilen kelimenin Türkçe karşılığını (self.turkce_sutun) içeren veri çerçevesinden (self.veri) alır ve dogru_cevap adlı bir değişkene atar. İşte daha ayrıntılı açıklama:
             #.loc[self.secilen_index, self.turkce_sutun]: Bu ifade, self.veri veri çerçevesinin belirli bir konumundan veri seçer. self.secilen_index indeksi ve self.turkce_sutun sütun adı kullanılarak, seçilen kelimenin Türkçe karşılığını (dogru_cevap) alır.
    ...
    def baslik_yaz(self):
        # Yanlış cevap durumunda metni oluştur
        metin = f"<div style='text-align: center;'><font color='red' style='font-size: 20px;'></font><span style='color: yellow; font-size: 20px; font-weight: bold;'>İkinci hali</span></div><font>"
    ...
    def help(self):
        ...
        message_box.setIcon(QMessageBox.Information)
        message_box.setWindowTitle("Bilgi")
        message_box.setText("Excel dosyanızda iki sütun olacak .Birinci sütunun adı 'first' , ikinci Sütunun adı 'second' olmalıdır. Program size fiilin birinci halini vericek siz de ikinci halini cevap olarak yazıcaksınız.")
        message_box.exec_()
